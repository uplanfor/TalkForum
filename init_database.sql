...
CREATE TABLE `user` (
  ...
  `intro` varchar(64) NOT NULL DEFAULT 'Hello~' COMMENT '用户的自我介绍',
  ...
  `avatar_link` varchar(128) NOT NULL DEFAULT 'https://pic1.imgdb.cn/item/6905cbfa3203f7be00bf6e98.webp' COMMENT '头像链接',
  `background_link` varchar(128) DEFAULT 'https://img-s.msn.cn/tenant/amp/entityid/BB1msDBR?w=0&h=0&q=50&m=6&f=jpg&u=t' COMMENT '背景链接（按规范设为可空）',
...
CREATE TABLE `club` (
  ...
  `description` varchar(128) DEFAULT NULL COMMENT '圈子描述',
  `avatar_link` varchar(128) NOT NULL DEFAULT '/icon.ico' COMMENT '头像链接（按规范设默认值）',
  `background_link` varchar(128) DEFAULT NULL COMMENT '背景链接',
