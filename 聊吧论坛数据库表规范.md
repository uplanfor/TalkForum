...
| fans_count | int | 粉丝数量 | 非空，默认0 |
| following_count | int | 关注数量 | 非空，默认0 |
| intro           | varchar(128) | 用户的自我介绍                  | 非空，默认“Hello~”     |
...
| avatar_link     | varchar(256) | 头像链接，默认为https://pic1.imgdb.cn/item/6905cbfa3203f7be00bf6e98.webp | 非空，默认"/logo.ico"  |
| background_link | varchar(256) | 背景链接 默认为https://img-s.msn.cn/tenant/amp/entityid/BB1msDBR?w=0&h=0&q=50&m=6&f=jpg&u=t | 可空                |
...
| title         | varchar(64) | 帖子标题                           | 可空                           |
| brief | varchar(128) | 帖子简介 | 非空 |
...
| root_id | bigint | 根评论ID（顶级评论为null） | 可空，索引 |
| parent_id  | bigint       | 关联回复的评论ID（顶级评论为null）           | 可空，索引                         |
| content    | varchar(1024) | 评论内容                           | 非空                            |
| status     | varchar(16)  | 状态（PENDING/PASS/REJECT/DELETE） | 非空，默认PENDING，索引               |
| created_at | datetime     | 创建时间                           | 非空，默认当前时间，索引                  |
| like_count | int          | 点赞数量                           | 非空，默认0                        |
| comment_count | int | 评论/回复数量 | 非空，默认0 |
...
| description     | varchar(512) | 圈子描述           | 可空                                |
| avatar_link     | varchar(256) | 头像链接           | 非空，默认为/icon.ico                   |
| background_link | varchar(256) | 背景链接           | 可空                                |
...
| interact_content     | tinyint     | 互动内容                                             | 非空，索引                                                   |
...
| interact_date        | datetime    | 互动时间                                             | 非空，默认当前时间，索引                                     |
| -                    | -           | -                                                    | 复合索引（interact_target_type, interact_target, interact_content） |
| -                    | -           | -                                                    | 复合唯一索引（user_id, interact_target_type, interact_target, interact_content） |
...
| handled_by | bigint | 处理人userid | 可空，索引 |
