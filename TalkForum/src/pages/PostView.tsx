...
const PostView = () => {
    ...
    const handleCommentSend = useCallback(
        debounce(async () => {
            if (commentContentRef.current) {
                ...
                await commentPostComment(
                    post.id,
                    content,
                    commentTarget.rootId,
        ...
        [post.id, commentTarget.parentId, commentTarget.rootId]
    ); // 500ms的防抖延迟，防止多次点击发送
    ...
    return ok ? (
        <div className='post-view-cover'>
            ...
            {/* 目录按钮 - 移动端 */}
            {tocNodeTree.length > 0 && (
                <button className='toc-mobile-button' onClick={() => setShowTocMenu(!showTocMenu)}>
                    <ChevronRightIcon />
                    {t('postView.contents')}
                </button>
            )}
            ...
            {showTocMenu && tocNodeTree.length > 0 && (
                <div className='toc-mobile-menu'>
                    ...
                    <div className='toc-mobile-content'>{renderTocTree(tocNodeTree)}</div>
                </div>
            )}
            ...
            <div className='comment-input'>
                {commentTarget.parentId && commentTarget.userId && (
                    <div className='comment-reply-show'>
                        <p>
                            <span>
                                {t('postView.replyTo')}{' '}
                                {getSingleSimpleUserInfo(commentTarget.userId).name}:{' '}
                            </span>
                ...
                <textarea
                    placeholder={
                        isLoggedIn ? t('postView.leaveComment') : t('postView.loginToComment')
                    }
            ...
            {showReportDialog &&
                createPortal(
                    <ReportDialog
                        ...
                        reportId={post.id}
                        reportTargetType={ReportTargetConstant.POST}
                        notification={t('postView.reportNotification', {
                            postId: post.id,
                            authorName: getSingleSimpleUserInfo(post.userId).name,
                            authorId: post.userId,
                            brief:
                                post.brief.length > 16
                                    ? post.brief.substring(0, 16) + '...'
                                    : post.brief,
                        })}
                    />,
                    document.body
