        ...
        <div className='space-view-cover'>
            <div className='space-header'>
                ...
                <EllipsisHorizontalIcon
                    onClick={async e => {
                        const menus = ['Share Space', 'Report'];
                        const res = await Msg.menu(menus);
                        if (res != -1) {
                            if (res == 0) {
                                copyToClipboard(String(window.location));
                                Msg.success(
                                    'Already copy the link at the copyboard!Send to friends to recommand it!'
                                );
                            } else if (res == 1) {
            ...
            <InfoBackground targetId={spaceId} targetType={getInfoBackgroundType()} />
            <PostContainer targetType={validType} targetId={spaceId} />
                ...
                createPortal(
                    validType == PostContainerTargetType.USER ? (
                        <ReportDialog
                            ...
                            reportId={spaceId}
                            reportTargetType={ReportTargetConstant.USER}
                        ...
                        <ReportDialog
                            onClose={() => setShowReportDialog(false)}
                            reportId={spaceId}
                            reportTargetType={ReportTargetConstant.CLUB}
