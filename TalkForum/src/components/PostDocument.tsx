...
const PostDocument = (props: PostDocumentProps) => {
                ...
                if (isRefreshingRef.current) {
                    ...
                    const newComments = res.data.data.map(item => {
                        ...
                        return {
                            ...
                            interactContent: existingComment
                                ? existingComment.interactContent
                                : item.interactContent,
                        };
                    });
                        ...
                        newComments.forEach(newComment => {
                            ...
                            if (existingCommentIndex === -1) {
                                ...
                                updatedComments[existingCommentIndex] = {
                                    ...newComment,
                                    interactContent:
                                        updatedComments[existingCommentIndex].interactContent,
                                };
                            }
                        });
    ...
    const handleLike = async (id: number) => {
        ...
        // 调用互动API
        await interactionMakeInteractionWithPost(id, targetInteractContent)
            .then(res => {
                if (res.success) {
                    // 更新本地状态
                    if (isLiked) {
            ...
            <div className='post-window'>
                    ...
                    <div className='author-info'>
                        <img
                            src={
                                isUserInfoLoading
                                    ? DefaultAvatarUrl
                                    : getSingleSimpleUserInfo(userId).avatarLink
                            }
                            alt='Avatar'
                        ...
                        <div className='author-combo'>
                            <span className='author-name'>
                                {isUserInfoLoading
                                    ? t('postDocument.loading')
                                    : getSingleSimpleUserInfo(userId).name}
                            </span>
                            <span className='post-date'>
                                {isEssence != 0 && (
                                    <span className='badge'>{t('postDocument.essence')}</span>
                                )}
                                {t('postDocument.lastEditedAt')}{' '}
                                {dayjs(updatedAt).format('HH:mm:ss MM DD, YYYY')}
                            </span>
                        </div>
                    </div>
                ...
                <div className='post-comment'>
                    ...
                    <div>
                        ...
                        {isErrorRef.current && (
                            <div style={{ textAlign: 'center', marginTop: '20px' }}>
                                {t('postDocument.errorLoadingComments')}{' '}
                                <button onClick={handleReload}>{t('postDocument.retry')}</button>
                            </div>
                        )}
                    </div>
                </div>
