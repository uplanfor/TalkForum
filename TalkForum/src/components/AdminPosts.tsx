...
const AdminPosts = () => {
    ...
    const handleAuditPost = async (postId: number) => {
        ...
        await postsAdminAuditPost(postId, statusItem)
            .then(res => {
                if (res.success) {
                    ...
                    if (statusItem === PostCommentStatusEnum.DELETE) {
                        ...
                        setPosts(prevPosts =>
                            prevPosts.map(post =>
                                post.id === postId ? { ...post, status: statusItem } : post
                            )
                        );
                    }
                } else {
    ...
    const handleToggleEssence = async (postId: number, currentEssence: number) => {
        ...
        const result = await Msg.confirm(t('adminPosts.essenceConfirm', { action }));
    ...
    const handleSetEssence = async (postId: number, isEssence: number) => {
        ...
        await postsAdminSetPostAsEssence(postId, isEssence)
            .then(res => {
                if (res.success) {
                    ...
                    // 局部更新帖子的精华状态
                    setPosts(prevPosts =>
                        prevPosts.map(post => (post.id === postId ? { ...post, isEssence } : post))
                    );
                } else {
                    throw new Error(res.message);
                }
    ...
    return (
        <div className='admin-posts-container'>
            ...
            <div className='action-buttons'>
                ...
                >
                    {t('adminPosts.batchAudit')}{' '}
                    {selectedPosts.length > 0 &&
                        `(${selectedPosts.length} ${t('adminPosts.selected')})`}
                </button>
                <button className='btn btn-success' onClick={() => setShowAuditDialog(true)}>
            ...
            {/* 审核对话框 */}
            {showAuditDialog && <AuditPostDialog onClose={() => setShowAuditDialog(false)} />}
        </div>
    );
};
