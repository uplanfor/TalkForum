...
const AdminPosts = () => {
  ...
  const loadPosts = async () => {
    setLoading(true);
      ...
      await postsAdminGetPostList(params).then(res=>{
        if (res.success) {
    ...
    <div className="admin-posts-container">
      ...
      <div className="action-buttons">
        {selectedPosts.length > 0 && (
          <span className="selected-count">Selected: {selectedPosts.length} posts</span>
        )}
      </div>
      ...
      <ShowTable
        ...
        renderItem={(item) => (
          <tr>
            ...
            <td>{item.userId}</td>
            <td>{item.clubId == null ? 'No Club' : item.clubId}</td>
            <td>{item.status}</td>
            <td>
              <button 
