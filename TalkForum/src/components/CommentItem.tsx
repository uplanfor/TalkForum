...
const CommentItem = ({
    ...
    const handleMenuClick = useCallback(async () => {
        ...
        // 检查是否显示删除选项
        const canDelete =
            user.isLoggedIn && (user.role !== UserRoleEnum.USER || userId === user.id);
        ...
        <div className='comment-item'>
            ...
            <div className='comment-info'>
                ...
                <div className='comment-header'>
                    <h4 className='comment-user-name' onClick={() => openSpaceView(userId)}>
                ...
                <p className='comment-footer'>
                    <span className='comment-footer-time'>
                        {dayjs(createdAt).format('HH:mm, MMM DD, YYYY')}{' '}
                    </span>
                    <span
                        className='comment-footer-reply'
                        onClick={() => {
                            // 设置评论目标，用于回复当前评论
                            setCommentTarget({
                                parentId: id,
                ...
                {commentCount > 0 && (
                    <div className='comment-reply-list'>
                        ...
                        {/* 回复总数和操作按钮 */}
                        <p className='reply-total'>
                            {commentCount} {t('commentItem.replies')}
                            {/* 展开/折叠按钮 */}
                            {
                                <span onClick={changeFoldReplyList}>
                                    {notFoldReplyList
                                        ? t('commentItem.fold')
                                        : t('commentItem.unfold')}
                                </span>
                            }
                            {/* 加载更多按钮（如果还有更多回复） */}
                            {hasMore && (
                                <span onClick={throttleLoadMoreReplies}>
                                    {t('commentItem.seeMore')}
                                </span>
                            )}{' '}
                        </p>
                    </div>
