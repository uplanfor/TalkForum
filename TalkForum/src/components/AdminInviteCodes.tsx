...
import { adminGetInvitecodeByPage, adminDeleteInviteCodes } from '../api/ApiInvitecode';
...
const AdminInviteCodes = () => {
    ...
    const loadInviteCodes = async (pageNum: number, size: number) => {
        setLoading(true);
        await adminGetInvitecodeByPage(pageNum, size)
            .then(async res => {
                if (res.success) {
    ...
    return (
        <div className='admin-invite-codes-container'>
            ...
            <div className='pagination-info'>
                <span>
                    {t('adminInviteCodes.showingEntries', {
            ...
            <ShowTable
                ...
                renderItem={item => (
                    <tr>
                        ...
                        <td>{item.code}</td>
                        <td>
                            {getSingleSimpleUserInfo(item.creatorId).name}({item.creatorId})
                        </td>
                        <td>{dayjs(item.createdAt).format('HH:mm MMM DD, YYYY')}</td>
                        <td>
                            {item.usedCount}/{item.maxCount}
                        </td>
