...
const InfoBackground = (props: InfoBackgroundProps) => {
    ...
    let isSelf =
        ...
        (targetType === InfoBackgroundType.USER && isLoggedIn && targetId == id);
    ...
    const handleFollow = async () => {
        if (isLoggedIn) {
            if (targetId != null) {
                ...
                await interactionsFollowOrUnfollowUser(targetId, !isCurrentlyFollowing)
                    .then(res => {
                        if (res.success) {
                            ...
                            if (isSelf) {
                                ...
                                setOtherUser(prev => {
                                    ...
                                    return {
                                        ...prev,
                                        fansCount: isCurrentlyFollowing
                                            ? prev.fansCount - 1
                                            : prev.fansCount + 1,
                                    };
                                });
                ...
                <div className='info-container'>
                    <div className='info'>
                        ...
                        <div className='info-combo'>
                            ...
                            {/* 默认用户详细信息 */}
                            <p>
                                {t('infoBackground.id')}: {defaultUserInfo.id}
                                <br />
                                {defaultUserInfo.followingCount} {t('infoBackground.following')}{' '}
                                {defaultUserInfo.fansCount} {t('infoBackground.followers')}
                            </p>
                        </div>
                    </div>
                    {/* 默认用户简介 */}
                    <div className='intro'>{t('infoBackground.signInPrompt')}</div>
                </div>
            </BackgroundImg>
        );
    }
            ...
            <div className='info-container'>
                <div className='info'>
                    ...
                    {targetType === InfoBackgroundType.SELF ? (
                        ...
                        <button
                            className={`info-interact-button ${following.includes(targetId) ? 'following' : 'follow'}`}
                            onClick={handleFollow}
                        >
                            {following.includes(targetId)
                                ? t('infoBackground.following')
                                : t('infoBackground.follow')}
                        </button>
                    )}
                    {/* 用户头像 */}
              ...
              */}
                        <p>
                            {' '}
                            {t('infoBackground.id')}: {userInfo.id}{' '}
                            {isSelf && `${t('infoBackground.email')}: ${userInfo.email}`} <br />
                            {isLoggedIn && userInfo.lastLoginAt && (
                                <>
                                    {t('infoBackground.lastLoginAt')}:{' '}
                                    {dayjs(userInfo.lastLoginAt).format('HH:mm:ss MMMM DD, YYYY')}
                                    <br />
                                </>
                            )}
                            {userInfo.followingCount} {t('infoBackground.following')}{' '}
                            {userInfo.fansCount} {t('infoBackground.followers')}
                        </p>
                    </div>
                </div>
                {/* 用户简介 */}
