...
import { postsAdminGetPostList, postsAdminAuditPost, type PostType, postsAdminGetPostContent } from '../api/ApiPosts';
...
const AuditPostDialog = ({ onClose }: AuditPostDialogProps) => {
    ...
    const loadPostQueue = useCallback(async (page: number) => {
        setIsLoading(true);
        try {
...
useEffect(() => {
  ...
  const handleRejectPost = useCallback(
    debounce(async () => {
      if (postPendingQueue.length === 0 || currentPostIndex >= postPendingQueue.length) return;
            ...
            {currentPost ? (
                <>
                    ...
                    <div style={{ textAlign: 'center', marginTop: '10px', fontSize: '14px', color: '#666' }}>
                        ...
                        {auditedPosts.has(currentPost?.id) && <span style={{ marginLeft: '10px', color: '#888' }}>(Reviewed)</span>}
                    </div>
                </>
            ) : (
                <div style={{ textAlign: 'center', padding: '20px' }}>
