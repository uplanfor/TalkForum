...
const AdminComments = () => {
    ...
    const handleBatchAuditComments = async () => {
        const menus = [
            ...
            PostCommentStatusEnum.DELETE,
        ];
        const index = await Msg.menu(
            menus,
            t('adminComments.batchAuditMenu', { count: selectedComments.length })
        );
        if (index === -1) {
            return;
        }
        ...
        // 调用管理员审核评论的API
        await commentAdminAuditComments(selectedComments, statusItem)
            .then(res => {
                if (res.success) {
                    Msg.success(
                        `${res.message}(${selectedComments.length} ${t('adminComments.comments')} ${statusItem})`
                    );
    ...
    const handleAuditComment = async (commentId: number) => {
        ...
        // 调用管理员审核评论的API
        await commentAdminAuditComments([commentId], statusItem)
            .then(res => {
                if (res.success) {
    ...
    return (
        <div className='admin-comments-container'>
                ...
                <div className='filter-row'>
                    <div className='filter-item'>
                        ...
                        >
                            ...
                            <option value={PostCommentStatusEnum.PASS}>
                                {t('adminComments.pass')}
                            </option>
                            <option value={PostCommentStatusEnum.REJECT}>
                                {t('adminComments.reject')}
                            </option>
                            <option value={PostCommentStatusEnum.PENDING}>
                                {t('adminComments.pending')}
                            </option>
                            <option value={PostCommentStatusEnum.DELETE}>
                    ...
                    <div className='filter-actions'>
                        <button className='search-button' onClick={throttledSearch}>
            ...
            <div className='action-buttons'>
                ...
                >
                    {t('adminComments.batchAudit')}{' '}
                    {selectedComments.length > 0 &&
                        `(${selectedComments.length} ${t('adminComments.selected')})`}
                </button>
                <button
            ...
            <div className='pagination-info'>
                <span>
                    {t('adminComments.showingEntries', {
            ...
            {/* 审核对话框 */}
            {showAuditDialog && <AuditCommentDialog onClose={() => setShowAuditDialog(false)} />}
        </div>
    );
};
