...
const PostContainer = ({
  ...
  useEffect(() => {
    ...
    const searchParamsString = targetType === PostContainerTargetType.SEARCH && searchParams
  ...
  useEffect(() => {
    ...
    if (targetType === PostContainerTargetType.SEARCH && hasInitialLoaded.current && searchParams) {
      // 重置状态
      updateCursor(null);
      setPosts([]);
      updateIsError(false);
  ...
  const loadMore = useCallback(async () => {
    ...
    if (!hasMoreRef.current || isErrorRef.current || isRefreshingRef.current || isLoadingRef.current) {
      console.log(!hasMoreRef.current, isErrorRef.current, isRefreshingRef.current, isLoadingRef.current, "hit!")
      return;
    }
    ...
    try {
      ...
      // 更新缓存
      setCache(targetType, targetId, searchParamsString, {
        posts: isRefreshingRef.current || cursorRef.current === null ? updatedPosts : [...posts, ...updatedPosts],
