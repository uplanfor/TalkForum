...
| `/comments/admin/audit` | PUT  | status（PASS/REJECT，直接传字符串字面量） commentIds涉及的所有评论id | code:200、message:"审核完成" | 管理员/风纪 | 更新comment（评论状态） |
...
| 路径                   | 方法 | 入参                                                         | 出参                                 | 权限    | 表操作               |
| ---------------------- | ---- | ------------------------------------------------------------ | ------------------------------------ | ------- | -------------------- |
| `/invitecode/admin` | GET  | page（页码）、pageSize（每页大小）                       | {total,data:[{code,status,createdTime}]} | 管理员 | 查询所有邀请码列表   |
| `/invitecode/admin` | POST | count（生成数量）、expireTime（过期时间）、maxUse（最大使用次数） | {codes:[{code,expireTime,maxUse}]} | 管理员 | 生成新邀请码         |
| `/invitecode/admin/{code}` | DELETE | 无（邀请码在路径参数中）                                             | code:200、message:"删除成功"          | 管理员 | 删除邀请码           |
