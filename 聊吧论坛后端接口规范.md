...
| 路径            | 方法 | 入参                                            | 出参                                           | 权限     | 表操作               |
| --------------- | ---- | ----------------------------------------------- | ---------------------------------------------- | -------- | -------------------- |
| `/user`         | POST | email（唯一）、password（密码）、name（用户名） | code:200、message:"注册成功"、userId（用户ID） | 无需登录 | 插入user（新建用户） |
...
| 路径           | 方法 | 入参                          | 出参             | 权限   | 表操作                  |
| -------------- | ---- | ----------------------------- | ---------------- | ------ | ----------------------- |
| `/invitecode/` | POST | max_count, count, expire_time | 生成的邀请码列表 | 管理员 | 对invite_code表新增项目 |
| `/invitecode`  | PUT  | status                        | code:200         | 管理员 | 设置邀请码是否可用      |
