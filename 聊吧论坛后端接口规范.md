...
| 路径                           | 方法 | 入参                                   | 出参                                 | 权限   | 表操作               |
| ------------------------------ | ---- | -------------------------------------- | ------------------------------------ | ------ | -------------------- |
| `/users/admin/`                | GET  | page（页码）、pageSize（数目，最大10） | users                                | 管理员 | 获取所有用户列表     |
...
| `/users/admin/{userId}/reset`  | PUT  | password（新密码）                     | code:200、message:"密码重置成功"     | 管理员 | 更新user（密码哈希） |
...
| `/posts`          | GET    | keyword（可选，关键词）、cursor（页码）、pageSize（数目，最大10）、clubIds（可选）、userIds（可选）、isEssence（可选）    游标分页形式 | {data（列表）,hasMore, cursor游标}                  | 无（筛选精选帖子，关注的人的帖子，某个圈子的帖子都是用这个接口） | 查询post（帖子列表） |
...
| 路径                            | 方法 | 入参                                                         | 出参                         | 权限        | 表操作               |
| ------------------------------- | ---- | ------------------------------------------------------------ | ---------------------------- | ----------- | -------------------- |
| `/posts/admin/`                 | GET  | keyword（可选，关键词）、page（页码）、pageSize（数目，最大10）、clubIds（可选）、userIds（可选）、isEssence（可选）status（帖子状态） | posts（列表）                | 管理员/风纪 | 查询post列表         |
...
| 路径              | 方法 | 入参                                                         | 出参                                   | 权限        | 表操作                                                       |
| ----------------- | ---- | ------------------------------------------------------------ | -------------------------------------- | ----------- | ------------------------------------------------------------ |
| `/reports/admin`  | GET  | page（页码）、pageSize(页面大小)、status（可选）、report_target_type（可选） | reports（全量举报列表）、total（总数） | 管理员/风纪 | 查询report（全量举报记录）                                   |
