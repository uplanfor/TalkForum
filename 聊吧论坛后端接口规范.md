...
| 路径                           | 方法 | 入参                                   | 出参                                 | 权限        | 表操作               |
| ------------------------------ | ---- | -------------------------------------- | ------------------------------------ | ----------- | -------------------- |
| `/users/admin/`                | GET  | page（页码）、pageSize（数目，最大10） | total data                           | 风纪/管理员 | 获取所有用户列表     |
...
| `/users/admin/{userId}/reset`  | PUT  | 无                                     | code:200、message:"密码重置成功"     | 风纪/管理员 | 更新user（密码哈希） |
...
互动类型interactType(POST/USER/COMMENT 直接传字符串) 如果为POST， 互动状况 -1表示踩，0表示没有互动，1表示点赞 对于POST,COMMENT情况，对于USER情况，0表示没有关注，1表示关注了

#### 9.1 互动操作（点赞/踩/关注）

| 路径            | 方法 | 入参                                                         | 出参                        | 权限   | 表操作                                                       |
| --------------- | ---- | ------------------------------------------------------------ | --------------------------- | ------ | ------------------------------------------------------------ |
| `/interactions` | POST | interactType（互动类型）、interactId（被互动对象ID）、interactContent互动的状况 | code:200、message:"success" | 需登录 | 1. 插入interaction（新建互动记录，若已存在则返回“已互动”）；<br>2. 若为点赞/踩：同步更新post/comment的like_count；<br>3. 若为关注：发送“被关注通知”给被关注用户 |

#### 9.2 互动记录查询

| 路径               | 方法 | 入参                                       | 出参                                                         | 权限   | 表操作                                |
| ------------------ | ---- | ------------------------------------------ | ------------------------------------------------------------ | ------ | ------------------------------------- |
| `/interactions/my` | GET  | interactType ,interactIds被互动的对象ids， | 互动状况 -1表示踩，0表示没有互动，1表示点赞 对于POST,COMMENT情况，对于USER情况，0表示没有关注，1表示关注了 | 需登录 | 查询interaction（本人互动记录），关联 |
...
| 路径                       | 方法   | 入参                                                         | 出参             | 权限   | 表操作                  |
| -------------------------- | ------ | ------------------------------------------------------------ | ---------------- | ------ | ----------------------- |
| `/invitecode/admin`        | GET    | page, pageSize                                               |                  | 管理员 |                         |
